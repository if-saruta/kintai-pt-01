                ...
                <div class="project-shfit-wrap">
                    ...
                    <div class="project-info-table">
                        <div class="project-info-table__row">
                            ...
                            <div class="project-info-table__row__txt --data"><p class="">{{$client->pdfName}}</p></div>
                        </div>
                        <div class="project-info-table__row">
                                            ...
                                            <div class="company-block">
                                                @foreach ($getCompanies as $getCompany)
                                                    <div class="company-item project{{$getCompany->id}}">
                                                        ...
                                                        <div class="company-item__clm retailClm"><p class="">上代</p></div>
                                                        <div class="company-item__clm expressClm"><p class="">高速代</p></div>
                                                        <div class="company-item__clm parkingClm"><p class="">パーキング代</p></div>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                            ...
                            <div class="project-shift-calender__data">
                                    ...
                                    <div class="project-shift-calender__data__row">
                                        ...
                                        @if ( $which_part == 0 )
                                            @foreach ( $projects as $project )
                                                <div class="project-shift-calender__data__row__item">
                                                    @foreach ( $getCompanies as $getCompany )
                                                        <div class="project-shift-calender__data__row__item__company project{{$getCompany->id}}">
                                        ...
                                        @foreach ($projects as $project)
                                            @foreach ($getCompanies as $getCompany)
                                                ...
                                                ?>
                                                {{-- 給与 --}}
                                                <div class="project-shift-calender__data__row__item project{{$getCompany->id}} salaryClm">
                                                    @foreach ( $ShiftProjectVehicles as $spv )
                                                        @if ($spv->shift->date == $date->format('Y-m-d'))
                                                ...
                                                {{-- 上代 --}}
                                                <div class="project-shift-calender__data__row__item --clm project{{$getCompany->id}} retailClm">
                                                    @foreach ( $ShiftProjectVehicles as $spv )
                                                        @if ($spv->shift->date == $date->format('Y-m-d'))
                                                            @if ($spv->shift->employee->company->id == $getCompany->id && $project->id == $spv->project_id)
                                                ...
                                                {{-- 高速代 --}}
                                                <div class="project-shift-calender__data__row__item --clm project{{$getCompany->id}} expressClm">
                                                    @foreach ( $ShiftProjectVehicles as $spv )
                                                        @if ($spv->shift->date == $date->format('Y-m-d'))
                                                            @if ($spv->shift->employee->company->id == $getCompany->id && $project->id == $spv->project_id)
                                                ...
                                                {{-- パーキング代 --}}
                                                <div class="project-shift-calender__data__row__item --clm project{{$getCompany->id}} parkingClm">
                                                    @foreach ( $ShiftProjectVehicles as $spv )
                                                        @if ($spv->shift->date == $date->format('Y-m-d'))
                                                            @if ($spv->shift->employee->company->id == $getCompany->id && $project->id == $spv->project_id)
                                ...
                                <div class="project-shift-calender__data__row">
                                    ...
                                    @foreach ($part_count as $name => $count)
                                        <div class="project-shift-calender__data__row__item --projectTotalCount --column">
                                            <p class="">{{$name}}</p>
                                            <p class="">{{$count}}</p>
                                        </div>
                                    @endforeach
            ...
            @if ($ShiftProjectVehicles !== null && !$ShiftProjectVehicles->isEmpty())
                <div class="PDF-dump --project-pdf">
                    <form action="{{route('invoice.project-edit-pdf')}}" method="POST">
                        ...
                        <input hidden type="text" name="projectClientNameByPdf" value="{{$client}}">
                        ...
                        <div class="view-clm">
                            @foreach ($getCompanies as $getCompany)
                                <div class="view-clm__item">
                                    ...
                                    <input type="checkbox" name="company_check[]" value="project{{$getCompany->id}}" class="viewClmCheck" checked>
                                </div>
                            @endforeach
                            <div class="view-clm__item">
                                <label for="">上代</label>
                                <input type="checkbox" name="retail_check" value="retailClm" class="viewClmCheck" checked>
                            </div>
                            <div class="view-clm__item">
                                <label for="">ドライバー</label>
                                <input type="checkbox" name="slary_check" value="salaryClm" class="viewClmCheck" checked>
                            </div>
                            <div class="view-clm__item">
                                <label for="">高速代</label>
                                <input type="checkbox" name="expressway_check" value="expressClm" class="viewClmCheck" checked>
                            </div>
                            <div class="view-clm__item">
                                <label for="">パーキング代</label>
                                <input type="checkbox" name="parking_check" value="parkingClm" class="viewClmCheck" checked>
                            </div>
                        </div>
                    </form>
